...
export class MessageParser {
    ...
    private static createContactLookup(contacts: EnhancedContact[]): Map<string, EnhancedContact> {
        ...
        contacts.forEach(contact => {
            // 优先使用原始ID和用户名进行精确匹配
            const primaryIdentifiers = [contact.originalId, contact.username].filter(Boolean);
            ...
            // 优先设置主要标识符
            primaryIdentifiers.forEach(id => {
                if (id) lookup.set(id, contact);
            });
            
            // 只有在主要标识符不存在时才使用次要标识符
            if (primaryIdentifiers.length === 0) {
                secondaryIdentifiers.forEach(id => {
                    if (id) lookup.set(id, contact);
                });
            }
        });
    ...
    private static isMessageForContact(message: EnhancedMessage, targetContact: EnhancedContact): boolean {
        ...
        const { originalId, username, displayName } = targetContact;
        
        // 优先使用原始ID进行精确匹配
        const identifiers = [originalId, username].filter(Boolean);
        
        // 精确匹配原始标识符
        const exactMatch = identifiers.some(identifier => 
            senderId === identifier || receiverId === identifier
        );
        
        if (exactMatch) return true;
        
        // 如果精确匹配失败，尝试包含匹配（用于群聊等复杂场景）
        const containsMatch = identifiers.some(identifier => 
            senderId.includes(identifier!) || 
            (receiverId && receiverId.includes(identifier!))
        );
        ...
        if (displayName && displayName.length > 3) {
            return senderId.includes(displayName) || 
                   (receiverId && receiverId.includes(displayName));
        }

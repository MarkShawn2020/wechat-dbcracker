...
export class WeChatTableMatcher {
    ...
    private static extractContactIdentifiers(contact: any): string[] {
        ...
        const possibleFields = [
            'mNsUsrName',      // 来自解析器的标准化字段名
            'originalId',      // 备用标识符
            'id',              // 联系人ID
            'username',        // 用户名
            'm_nsUsrName'      // 原始字段名（大小写敏感）
        ];
        ...
        // 如果没有找到任何标识符，尝试使用displayName作为备用
        if (identifiers.length === 0 && contact.displayName) {
            identifiers.push(contact.displayName);
        }

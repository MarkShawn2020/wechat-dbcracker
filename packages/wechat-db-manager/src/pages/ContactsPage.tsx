...
export function ContactsPage() {
    ...
    // 处理联系人点击
    const handleContactClick = (contact: EnhancedContact) => {
    ...
    const loadContacts = useCallback(async () => {
        ...
        try {
            ...
            let loadedContacts: EnhancedContact[];

            // 如果有消息数据库，使用智能排序（包含活跃度信息）
            if (messageDbs.length > 0) {
                loadedContacts = await ChatDataService.loadContactsWithHeuristicSorting(
                    contactDb,
                    messageDbs,
                    undefined, // 不显示进度
                    undefined  // 不支持取消
                ...
                // 否则使用基础加载
                loadedContacts = await ChatDataService.loadContacts(contactDb);
                                ...
                                contact.contactType === 'group' ? '群聊' : '公众号'}
                                                    ...
                                                    {contact.lastActiveTime && (
                                                        <div className="flex items-center space-x-1">
                                                            <div
                                                                className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
                                                            <span className="text-xs text-green-600">有消息</span>
                                                        </div>
                                                    )}
                                                </div>
                                                ...
                                                <div
                                                    ...
                                                    <span className="text-xs text-blue-500">点击查看聊天记录</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
